...
datasets_dir="/home/jack/work/superposition/sparse_datasets"
...
for dataset_path in $datasets_dir/*; do
	...
	if [ -d "$RESULTS_DIR/initial_probes/pythia-1b-deduped/$dataset_name/average_activation" ]; then
		echo "Skipping $dataset_name"
		continue
	fi
	# Run the Python command for each dataset
	echo "about to call python"
	python3 probing_experiment.py --feature_dataset "$dataset_name" --experiment_type average_activation --experiment_name initial_probes --model pythia-1b-deduped
done
